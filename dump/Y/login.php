    ...
    if (isset($_POST['submit'])){
      $pesan =''; $redirect='';
        ...
        if (empty($cek)){
          ...
          echo "<SCRIPT> alert('Login gagal. Username Belum Terdaftar');window.location='login.php'</SCRIPT>";
        } else {
          if ($password != $cek['password']){
            echo "<SCRIPT> alert('Login gagal. Kata Sandi Salah');window.location='login.php'</SCRIPT>";
          } else {
            session_start();
            $_SESSION['id_user'] = $cek['id_user'];
            $_SESSION['username'] = $username;
            echo "<SCRIPT> alert('Login Sukses');window.location='indexUser.php'</SCRIPT>";
          }
        }
      }
    ?>