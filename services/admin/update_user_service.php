...
if (empty($username) || empty($firstname) || empty($lastname) || empty($email) || empty($nohp)) {
  echo "<SCRIPT>alert('Update profile gagal');window.location='../../admin-user.php'</SCRIPT>";
  return;
}
try {
...
echo "<SCRIPT>alert('Update profile sukses');window.location='../../admin-user.php'</SCRIPT>";
